...
# Populated after FSx is created; setup scripts wait and retry until available.
resource "aws_ssm_parameter" "fsx_dns" {
  name       = "${local.ssm_prefix}/fsx-dns"
  type       = "String"
  value      = aws_fsx_windows_file_system.shared.dns_name
  depends_on = [aws_fsx_windows_file_system.shared]
}
