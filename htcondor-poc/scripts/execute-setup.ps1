...
# ── Main ─────────────────────────────────────────────────────────────────────
$stage = Get-Stage
Log "=== Execute (compute-0) setup starting at stage $stage ==="

switch ($stage) {

    # ── Stage 0: rename + set DNS + reboot ──────────────────────────────────
    0 {
        Log "Stage 0: Rename to 'compute-0', set DNS to DC"
        ...
        if ($env:COMPUTERNAME -ne "COMPUTE-0") {
            Rename-Computer -NewName "COMPUTE-0" -Force
            Set-Stage 1
    ...
    1 {
        ...
        if (-not (Wait-ForPort $dcIp 389 40)) {
            Log "DC not ready after 40 min" "ERROR"; exit 1
        }
        ...
        if (-not $?) { Add-Computer -DomainName $domain -Credential $cred -Force }
    ...
    # ── Stage 2: done ────────────────────────────────────────────────────────
    2 {
        Log "Stage 2: domain-join complete; execute node is idle at v2."
        Set-Stage 99
        Unregister-ScheduledTask -TaskName "HTCondorSetup" -Confirm:$false -ErrorAction SilentlyContinue
        "Execute v2 setup complete $(Get-Date)" | Set-Content "C:\SetupComplete.txt"
        Log "=== Execute setup COMPLETE (v2) ==="
    }
