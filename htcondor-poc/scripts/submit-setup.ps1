...
# ── Main ─────────────────────────────────────────────────────────────────────
$stage = Get-Stage
Log "=== Submit (ws-0) setup starting at stage $stage ==="

switch ($stage) {

    # ── Stage 0: rename + set DNS + reboot ──────────────────────────────────
    0 {
        Log "Stage 0: Rename to 'ws-0', set DNS to DC"
        ...
        if ($env:COMPUTERNAME -ne "WS-0") {
            Rename-Computer -NewName "WS-0" -Force
            Set-Stage 1
    ...
    # ── Stage 2: done ────────────────────────────────────────────────────────
    2 {
        Log "Stage 2: domain-join complete; submit node is idle at v2."
        Set-Stage 99
        Unregister-ScheduledTask -TaskName "HTCondorSetup" -Confirm:$false -ErrorAction SilentlyContinue
        "Submit v2 setup complete $(Get-Date)" | Set-Content "C:\SetupComplete.txt"
        Log "=== Submit setup COMPLETE (v2) ==="
    }
